...
const COND_LABELS = {
  PREDICTIONS_MADE: 'Total Predictions Made', PREDICTION_STREAK: 'Prediction Streak',
  PREDICTION_POINTS: 'Prediction Points', CORRECT_PREDICTIONS: 'Correct Predictions',
  FANTASY_JOINS: 'Squad Challenges Joined', AGENT_CREATED: 'Agents Created',
  REPUTATION_SCORE: 'Reputation Score', NFTS_EARNED: 'NFTs Earned',
};

const emptyForm = {
  id: '', name: '', description: '', category: 'BADGE',
  condition_type: 'PREDICTIONS_MADE', condition_target: 1,
  reward_name: '', reward_description: '', reward_category: 3, max_claims: 0,
};

export default function FranchiseChallenges() {
  ...
  async function load() {
    try {
      const res = await fetch(`${API}/challenges?page=${page}&limit=25`, { headers: H(address) });
      if (res.ok) {
  ...
  function startEdit(c) {
    setForm({
      ...
      condition_type: c.condition_type, condition_target: c.condition_target,
      reward_name: c.reward_name || '', reward_description: c.reward_description || '',
      reward_category: CATEGORIES.indexOf(c.category), max_claims: c.max_claims || 0,
    });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await fetch(`${API}/challenges`, { method: 'POST', headers: H(address), body: JSON.stringify(payload) });
  ...
  async function handleDeactivate(id, name) {
    ...
    await fetch(`${API}/challenges/${id}`, { method: 'DELETE', headers: H(address) });
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {challenges.length === 0 && (
        <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-2xl p-12 text-center text-zinc-500 text-sm">No challenges yet. Create your first challenge above.</div>
      )}
