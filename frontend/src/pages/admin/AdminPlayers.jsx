...
export default function AdminPlayers() {
  ...
  // Derive unique teams from existing players
  const teams = [...new Set(players.filter(p => p.active).map(p => p.team))].sort();

  async function load() {
    try {
      // Load all players at once - ~160 PSL players, no need to paginate
      const res = await fetch(`/api/admin/players?limit=500`, { headers: H(address) });
      if (res.ok) {
  ...
  function startNew() {
    setForm({
      player_id: Math.max(0, ...players.map(p => p.player_id)) + 1,
  ...
  async function handleSubmit(e) {
    ...
    setMsg(null);
    try {
      const res = await fetch('/api/admin/players', { method: 'POST', headers: H(address), body: JSON.stringify(form) });
  ...
  async function handleDeactivate(pid, name) {
    ...
    await fetch(`/api/admin/players/${pid}`, { method: 'DELETE', headers: H(address) });
  ...
  const filteredTeams = Object.entries(allTeams)
    .filter(([team]) => filterTeam === 'ALL' || team === filterTeam)
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <select value={filterTeam} onChange={e => setFilterTeam(e.target.value)} className="input w-auto">
      ...
      {filteredTeams.length === 0 ? (
        ...
        <div key={team}>
          <h2 className="text-sm font-bold text-zinc-300 mb-2 flex items-center gap-2">
            {team}
            <span className="text-zinc-600 font-normal text-xs">({teamPlayers.filter(p => p.active).length} active)</span>
          </h2>
          <div className="bg-zinc-900/70 border border-zinc-800/80 rounded-2xl overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {teamPlayers.map(p => (
                  <tr key={p.player_id} className={`hover:bg-zinc-800/30 ${!p.active ? 'opacity-40' : ''}`}>
                    ...
                    <td className="px-4 py-2.5">{p.active ? <span className="text-emerald-400 text-2xs font-medium">Active</span> : <span className="text-red-400 text-2xs">Inactive</span>}</td>
